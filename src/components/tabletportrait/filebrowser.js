...
function Browse(props) {
  ...
  try {
    ...
    let buttons = jsonPath.map(item => {
      if (item.type === "dir") {
        return (
          <Link onClick={scrollToTop} className="link folder-link" key={`${item.name}`} to={`${url}/${item.name}`}><button className={`${props.format}folder-btn`} style={{height: "12vh", width: "20vw"}}><p style={{fontSize: "calc(1vw + 1vh)"}}>{item.name}</p></button></Link>
        );
      } else {
        return (
          <Link onClick={scrollToTop} className="link file-link" key={`${item.name}`} to={`/displayer?file=/${dirPath.join("/")}/${item.name}`}><button className={`${props.format}file-btn`} style={{height: "22vh", width: "20vw"}}><p style={{fontSize: "calc(1vw + 1vh)"}}>{item.name}</p></button></Link>
        );
      }
    });
